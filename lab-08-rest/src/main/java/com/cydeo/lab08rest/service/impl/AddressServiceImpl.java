...
public class AddressServiceImpl implements AddressService {
    ...
    public AddressDTO update(AddressDTO addressDTO) {
        ...
        address.setCustomer(mapperUtil.convert(
                customerService.findById(addressDTO.getCustomerId()), new Customer()));
    ...
    public List<AddressDTO> readByStartsWith(String address) {
       List<AddressDTO> list =addressRepository.findAllByStreetStartingWith(address).stream()
                .map(eachAddress->mapperUtil.convert(eachAddress, new AddressDTO()))
                .collect(Collectors.toList());
    ...
    public List<AddressDTO> readAllByCustomerId(Long id) {
        List<AddressDTO> list =addressRepository.retrieveByCustomerId(id).stream()
                .map(eachAddress->mapperUtil.convert(eachAddress, new AddressDTO()))
                .collect(Collectors.toList());
    ...
    public List<AddressDTO> readAllByCustomerIdAndName(Long customerId, String name) {
        List<AddressDTO> list =addressRepository.findAllByCustomerIdAndName(customerId,name).stream()
                .map(eachAddress->mapperUtil.convert(eachAddress, new AddressDTO()))
                .collect(Collectors.toList());
