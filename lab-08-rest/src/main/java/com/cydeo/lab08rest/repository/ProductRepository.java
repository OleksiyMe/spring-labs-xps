...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query(value = "select * from product p where p.price > ?1 and p.remaining_quantity < ?2",
            nativeQuery = true)
    List<Product> retrieveProductListGreaterThanPriceAndLowerThanRemainingQuantity(
            BigDecimal price, int remainingQuantity);
    ...
    @Query(value = "select * from product p join product_category_rel pl on pl.p_id = p.id " +
            "where pl.c_id in(?1) and p.price > ?2 " , nativeQuery = true)
