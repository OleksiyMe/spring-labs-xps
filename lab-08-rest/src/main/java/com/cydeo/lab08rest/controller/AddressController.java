...
public class AddressController {
    ...
    public ResponseEntity<ResponseWrapper> getAddressByStartsWith(
            @PathVariable("address") String address) {
        return ResponseEntity.ok(new ResponseWrapper(
                "Addresses are retrieved",
                addressService.readByStartsWith(address),
                HttpStatus.OK
        ));
    }

    @GetMapping("/customer/{id}")
    public ResponseEntity<ResponseWrapper> getAddressByCustomerId(
            @PathVariable("id") Long id) {
        return ResponseEntity.ok(new ResponseWrapper(
                "Addresses are retrieved",
                addressService.readAllByCustomerId(id),
                HttpStatus.OK
        ));
    }
    ...
    public ResponseEntity<ResponseWrapper> getAddressByStartsWith(
            @PathVariable("customerId") Long customerId, @PathVariable("name")String name) {
        return ResponseEntity.ok(new ResponseWrapper(
                "Addresses are retrieved",
                addressService.readAllByCustomerIdAndName(customerId,name),
                HttpStatus.OK
        ));
    }
